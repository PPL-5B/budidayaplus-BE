...
from fish_death.schemas import FishDeathCreateSchema, FishDeathOutputSchema
...
class FishDeathSchemaTest(TestCase):
    def setUp(self):
        ...
        self.pond = Pond.objects.create(
            owner=self.user, name='Test Pond', image_name='test_pond.png',
            length=10.0, width=5.0, depth=2.0
        )
        self.cycle = Cycle.objects.create(
            supervisor=self.user, start_date=start_time, end_date=end_time
        )
        self.fish_death = FishDeath.objects.create(
            pond=self.pond, reporter=self.user, cycle=self.cycle,
            fish_death_count=10, fish_alive_count=90, recorded_at=self.now
        )
        self.second_fish_death = FishDeath.objects.create(
            pond=self.pond, reporter=self.user, cycle=self.cycle,
            fish_death_count=5, fish_alive_count=85, recorded_at=self.now
        )
    ...
    def test_create_schema_valid_data(self):
        ...
        self.assertEqual(schema.fish_death_count, 10)
        self.assertEqual(schema.fish_alive_count, 90)

    def test_create_schema_invalid_fields(self):
        invalid_cases = [
            {"fish_death_count": "not a number", "fish_alive_count": 90, "recorded_at": self.now},
            {"fish_death_count": 10, "fish_alive_count": "not a number", "recorded_at": self.now},
        ]
        for case in invalid_cases:
            with self.subTest(case=case):
                with self.assertRaises(ValidationError):
                    FishDeathCreateSchema(**case)
    ...
    def test_output_schema_from_model(self):
        ...
        self.assertEqual(output_schema.id, self.fish_death.id)
