...
#TESTING UNTUK FITUR POP UP NOTIF
def test_get_pond_quality_alerts_no_data(self):
    #Kasus kalau gaada data
...
def test_get_pond_quality_alerts_all_parameters_meet_target(self):
    # Parameter memenuhi target --> gaada alert
    PondQuality.objects.create(
        pond=self.pond,
        reporter=self.user,
        cycle=self.cycle,
        ph_level=7.5,  # Sesuai target
        salinity=30.0,  # Sesuai target
    ...
    self.assertEqual(response.status_code, 200)
    self.assertEqual(len(response.json()), 0)  # Tidak ada alert
